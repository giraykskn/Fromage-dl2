...
class FromageModel(nn.Module):
  def __init__(self, tokenizer, args: FrozenArgs = FrozenArgs()):
    ...
    #print("Distributing LM to all GPUs:")
    #self.lm = torch.nn.DataParallel(self.lm, device_ids=[0, 1, 2, 3])
    
    #print("Distributing VM to all GPUs:")
  ...
  def generate_for_images_and_texts(
    ...
    if len(all_ret_idx) == 0:
      ...
      for ret_idx in all_ret_idx:
        ...
        _, top_image_idx = scores.squeeze().topk(max_img_per_ret + 6)
        image_outputs = []
...
def load_fromage(model_dir: str) -> Fromage:
  ...
  checkpoint = torch.load(model_ckpt_path, map_location=torch.device('cuda'))
